...
interface PredictionProps {
  ...
  onBusDetected: (croppedImage: string, detections: { x: number; y: number; width: number; height: number; confidence: number }[]) => void;
}
...
export const usePrediction = ({
  ...
  const doPredictFrame = useCallback(async (imageData: string) => {
